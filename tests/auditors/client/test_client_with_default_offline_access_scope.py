...
class TestClientWithDefaultOfflineAccessScope:
    @pytest.fixture
    def auditor(self, database, default_config):
        auditor_instance = ClientWithDefaultOfflineAccessScope(database, default_config)
    ...
    def test_client_can_generate_offline_tokens(
        self, mock_client, auditor, default_scopes, flow_status, use_refresh_tokens, expected
    ):
        mock_client.get_default_client_scopes.return_value = default_scopes
    ...
    def test_audit_function_with_findings(self, mock_client, auditor):
        mock_client.get_default_client_scopes.return_value = ["offline_access"]
        ...
        finding = results[0]
        assert finding.to_dict()["additional_details"]["default_scopes"] == ["offline_access"]
    ...
    def test_audit_function_multiple_clients(self, auditor):
        ...
        # We need to do this because the mocked functions are called more than once.
        # This means that the `side_effect` method used in other test functions fails.
        client1 = Mock()
        client1.get_default_client_scopes.return_value = ["offline_access"]
        ...
        results = list(auditor.audit())
        assert len(results) == 2  # Expect findings from two clients (client1 and client3)
