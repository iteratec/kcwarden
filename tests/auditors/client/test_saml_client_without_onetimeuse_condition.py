...
from kcwarden.auditors.client.saml_client_without_onetimeuse_condition import SamlClientWithoutOneTimeUseCondition


class TestSamlClientWithoutOneTimeUseCondition:
    @pytest.fixture
    def auditor(self, database, default_config):
        auditor_instance = SamlClientWithoutOneTimeUseCondition(database, default_config)
    ...
    def test_audit_function_secure_client(self, mock_client, auditor):
        ...
        mock_client.is_saml_onetimeuse_condition_enabled.return_value = True
    ...
    def test_audit_function_vulnerable_client(self, mock_client, auditor):
        mock_client.is_saml_client.return_value = True
        mock_client.is_saml_onetimeuse_condition_enabled.return_value = False
        auditor._DB.get_all_clients.return_value = [mock_client]
        results = list(auditor.audit())
