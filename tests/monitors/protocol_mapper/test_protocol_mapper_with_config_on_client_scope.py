...
class TestProtocolMapperWithConfigOnClientScope:
    ...
    def test_audit_scope_match_unused(self, monitor, mock_scope, mock_protocol_mapper):
        mock_scope.get_protocol_mappers.return_value = [mock_protocol_mapper]
        monitor._DB.get_all_scopes.return_value = [mock_scope]
        monitor._DB.get_all_clients.return_value = []
        monitor.get_custom_config = Mock(
            return_value=[
                {
        ...
        assert details["used_by_clients"] == []
        assert details["scope_is_used_by_any_client"] is False

    def test_audit_scope_match_used_by_client(self, monitor, mock_scope, mock_client, mock_protocol_mapper):
        mock_scope.get_protocol_mappers.return_value = [mock_protocol_mapper]
